...
int llopen(linkLayer connectionParameters){
    ...
    newtio.c_cc[VTIME]    = 1;   //Desativa o tempo de espera entre caracteres, assim que ler um caracter está pronto para o proximo sem ter de esperar
    newtio.c_cc[VMIN]     = 0;   //Aguarda até ser recebido um caracter e não retorna até tal acontecer
...
int llwrite(char* buf, int bufSize){
    ...
    while(1) { //Repete até finalizar a trasferencia em que recebe o RR1
        ...
        //Recebe a trama RR0
        if(AUX_Frame[0] == FLAG && AUX_Frame[1] == A0 && AUX_Frame[2] == RR0 && AUX_Frame[3] == (A0 ^ RR0) && AUX_Frame[4] == FLAG && confirmation>0) {
            frame[2] = I1; //substitui na frame os bytes necessarios para enviar a segunda trama I
            frame[3] = A0 ^ I1;
            transmition = frame_transmition(fd, frame, f); //envia a trama I
            if(transmition < 0) return -1;
        } 
        //Recebe a trama RR1
        else if(AUX_Frame[0] == FLAG && AUX_Frame[1] == A0 && AUX_Frame[2] == RR1 && AUX_Frame[3] == (A0 ^ RR1) && AUX_Frame[4] == FLAG && confirmation > 0) {
            break; //Quando recebe RR1 finaliza
        } 
        //Recebe a trama REJ0
        else if(AUX_Frame[0] == FLAG && AUX_Frame[1] == A0 && AUX_Frame[2] == REJ0 && AUX_Frame[3] == (A0 ^ REJ0) && AUX_Frame[4] == FLAG && confirmation > 0) {
            transmition = frame_transmition(fd, frame, f); //Quando recebe a trama REJ0 reenvia a 1ª trama I pois nao recebeu corretamente
            if(transmition < 0) return -1;
        }
        //Recebe a trama REJ1
        else if(confirmation > 0 && AUX_Frame[0] == FLAG && AUX_Frame[1] == A0 && AUX_Frame[2] == REJ1 && AUX_Frame[3] == (A0 ^ REJ1) && AUX_Frame[4] == FLAG && confirmation > 0) {
